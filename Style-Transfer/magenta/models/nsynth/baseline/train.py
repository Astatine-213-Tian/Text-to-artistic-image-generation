...
tf.app.flags.DEFINE_string("model", "ae", "Which model to use in checkpoint/")
tf.app.flags.DEFINE_string("config",
                           "nfft_1024",
                           "Which config to use in checkpoint/configs/")
...
def main(unused_argv):
  ...
  with tf.Graph().as_default():
    ...
    model = utils.get_module("baseline.checkpoint.%s" % FLAGS.model)
    ...
    with tf.device(cpu_device):
      with tf.name_scope("Reader"):
        batch = reader.NSynthDataset(
            FLAGS.new_path, is_training=True).get_baseline_batch(hparams)
